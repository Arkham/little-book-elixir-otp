defmodule Pooly.PoolServer do
  ...
  defmodule State do
    defstruct pool_sup: nil, worker_sup: nil, monitors: nil, size: nil,
      workers: nil, name: nil, mfa: nil, waiting: nil, overflow: nil, max_overflow: nil
  end
  ...
  defp handle_worker_exit(_pid, state) do
    ...
    case :queue.out(waiting) do
      {{:value, {from, ref}}, left} ->
        new = new_worker(worker_sup)
        true = :ets.insert(monitors, {new, ref})
        GenServer.reply(from, new)
        %{state | waiting: left}

      # Dinamically created worker has exited. No need to recreate it
      {:empty, empty} when overflow > 0 ->
        %{state | overflow: overflow - 1, waiting: empty}

      # Worker was not created dinamically. We need to create another one
      {:empty, empty} ->
